...
<%@page import="com.pfs.bean.Comment"%>
...
<%
	...
	Examiner examiner = (Examiner)session1.getAttribute("examiner"); 
	ExaminerUtils eu=new ExaminerUtils();
	List<Comment> commlistclient=(ArrayList<Comment>)session1.getAttribute("commlistclient");
	List<Comment> commlistexaminer=(ArrayList<Comment>)session1.getAttribute("commlistexaminer");
...
%>
  ...
  <tr>
	...
	<td> <%=req.getRid()%> </td> <td> <%=req.getRdesc()  %></td> <td> <%=req.getRdate()%>  </td> <td> <%=req.getRstatus()%>  </td>
	</tr>
	
  	
</table>
        <%
        if(!commlistexaminer.isEmpty()){
        %>
        <h4> PREVIOUS EXAMINER COMMENTS </h4>
        <%	
              		for(Comment c : commlistexaminer){
              			 
              %>
              <label style="font-size: 20px; background-color: grey ; color:white;" > <%= c.getExaminer().getEname() %> :</label> &nbsp;&nbsp;&nbsp;&nbsp;<label style="font-size: 20px; "> <%= c.getComm_text() %> </label>
              <br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<label style="font-size: 10px ; color: grey; "> <%= c.getComm_date() %> </label>
              <br><br>
              <% 
              }
              ...
              %>
              <h4> NO PREVIOUS EXAMINER COMMENTS </h4>
              <br>
              <% } %>
              
              <% if(!commlistclient.isEmpty()){
            	  %>
          <h4> PREVIOUS CLIENT COMMENTS </h4>
        <%
              		for(Comment c : commlistclient){
              			 
              %>
              <label style="font-size: 20px; background-color: grey; color:white;" > <%= c.getClient().getCname() %> : </label> &nbsp;&nbsp;&nbsp;&nbsp;<label style="font-size: 20px; "> <%= c.getComm_text() %> </label>
              <br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<label style="font-size: 10px ; color: grey; "> <%= c.getComm_date() %> </label>
              <br><br>
              <% }
              ...
              %>
                  <h4> NO PREVIOUS CLIENT COMMENTS</h4><br> <br> 
                  <% } %>
        <h4> ENTER YOUR COMMENT </h4>
        <form action="addCommentExaminer" method="POST">
        <div class="form_settings">
         <!-- <p><span>RID: </span><input class="contact" type="text" name="rid" value="" /></p> -->
         <textarea name="comm_text" cols="40" style="width:500px;" rows="5" > </textarea>
            <br> <br> 
